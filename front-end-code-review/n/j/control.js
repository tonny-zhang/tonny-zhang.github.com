...
CJS.findScripts = function() {
	...
	console.log(len,CJS.aScripts.length);
...
CJS.processInlineScript = function(script) {
	...
	code = code.replace('text/javascript','text/cjs').replace(/(['"])text\/javascript\1/g,'$1text/cjs$1').replace(/src(\s*=\s*(['"])[^'"]*?\1)/g,'data-cjssrc$1')
	code = code.replace('%22text%2Fjavascript%22','%22text%2Fcjs%22');
// console.log('---',code);	
	CJS.eval(code);
};
